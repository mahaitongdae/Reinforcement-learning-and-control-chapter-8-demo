...
class Solver(Dynamics_Config):
    ...
    def structured_mpc_solver(self, x_init, predict_steps):
        # Open loop solution
        x = SX.sym('x', self.DYNAMICS_DIM)
        u = SX.sym('u', self.ACTION_DIM)
        ...
        for k in range(1, predict_steps + 1):
            ...
            ubg += self.zero
            w += [Xk]
            lbw += [-inf, -50, -pi / 2, -20, -inf]
            ubw += [inf, 50, pi / 2, 20, inf]
            # lbw += [-inf, -inf, -inf, -inf, -inf]
            # ubw += [inf, inf, inf, inf, inf]
            ...
            if k == 1:
                ...
                F_cost = Function('F_cost', [x, u], [0.1 * (x[0] - self.a_curve * sin(self.k_curve * x[4])) ** 2
                                                     + 0.1 * (x[2] - arctan(self.a_curve * self.k_curve * cos(self.k_curve * x[4]))) ** 2
                                                     + 0.001 * u[0] ** 2])
                ref_y = self.a_curve * sin(self.k_curve * Xk[4])
                ref_psi = arctan(self.a_curve * self.k_curve * cos(self.k_curve * Xk[4]))
            else:
                F_cost = Function('F_cost', [x, u], [0.1 * (x[0] - ref_y) ** 2
                                                     + 0.1 * (x[2] - ref_psi) ** 2
                                                     + 0.001 * u[0] ** 2])
            J += F_cost(w[k * 2], w[k * 2 - 1])
